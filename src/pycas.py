...
def do_redirect(cas_host, service_url, opt, secure, handler):
    ...
    if opt=="gateway":
        ...
        logging.info(make_pycas_cookie("gateway", domain, path, secure))
    logging.info('User not logged in, redirecting to: %s', cas_url)
    handler.redirect(cas_url, abort=True)
...
def validate_cas_2(cas_host, service_url, ticket, opt):
    ...
    logging.info('Cas validate: %s', cas_validate)
    if opt:
...
def get_ticket_status(cas_host,service_url,protocol,opt,web_handler):
    ticket = web_handler.request.get("ticket")
...
#        ticket = cgi.FieldStorage()["ticket"].value
        if protocol==1:
            ...
            logging.info('Received ticket status: %s', ticket_status)
...
#
def login(cas_host, service_url, web_handler, lifetime=None, secure=1, protocol=2, path="/", opt=""):
    ...
    ticket_status, id = get_ticket_status(cas_host,service_url,protocol,opt,web_handler)

    if ticket_status==TICKET_OK:
        timestr     = str(int(time.time()))
        hash_code        = makehash(timestr + ":" + id)
        cookie_val  = hash_code + timestr + ":" + id
    ...
    #  Do redirect
    do_redirect(cas_host, service_url, opt, secure, web_handler)
